var estadosCidades = {
  ...
  estados: [
    ...
    },
    {
      ...
      capital_pais: { capital: false, ano_inicio: 1763, ano_fim: 1960 },
      regiao: "Sudeste",
      cidades: [
        {
          nome: "Rio de Janeiro",
    ...
    },
    {
      ...
      nome: "Bahia",
      capital: "Salvador",
      capital_pais: { capital: false, ano_inicio: 1549, ano_fim: 1763 },
      regiao: "Nordeste",
      cidades: [
        {
          nome: "Salvador",
    ...
    },
    {
      ...
      capital: "Brasília",
      capital_pais: { capital: true, ano_inicio: 1960, ano_fim: false },
      regiao: "Centro-Oeste",
      cidades: [
        {
          nome: "Brasília",
...
//Siglas dos estados
const estado = estadosCidades.estados;
const getListaDeEstados = function (Json) {
  let listaEstados = Json.slice();
  let estadosArray = {};
  let estadosJson = [];
  let status;

  listaEstados.forEach(function (item) {
    estadosJson.push(item.sigla);
    estadosArray.UF = estadosJson;
  });
...
const getCapitalEstado = function (sigla) {
  let jsonListaCapitalEstados = {};
  let status = false;

  estado.forEach(function (estado) {
    if (estado.sigla == sigla) {
      jsonListaCapitalEstados = {
        uf: estado.sigla,
        descricao: estado.nome,
        capital: estado.capital,
      };
  ...
  if (status == true) return jsonListaCapitalEstados;
  else return false;
};

// Estados e Região
const getEstadosRegiao = function (regiaoEs) {
  let jsonListaDeEstadosPorRegiao = {};
  let status = false;
  let estados = [];
  let regiao = regiaoEs;

  estado.forEach(function (estadoRe) {
    let estado = estadoRe;
